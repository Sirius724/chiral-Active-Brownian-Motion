...
void g_6_Measure(int ri, double *r, double *g_6_host, int si, double *q, double *omega_p_host, double *omega_t_host, int rdf_count, double omega, double tau_p, double rho, int select_potential){
...
int main(int argc, char** argv){
  ...
  char* char_omega = argv[1];
  sscanf(char_omega, "%lf", &omega);

  char* char_rho = argv[2];
  sscanf(char_rho, "%lf", &rho); 
	
  char* char_pot = argv[3];
  ...
  double Pe_list[7] = {1.00, 2.00, 5.00, 10.00, 50.00, 100.00, 200.0};
  double cutoff_list[7] = {1.955, 1.985, 2.035, 2.065, 2.065, 2.065, 2.055};
  ...
  char filename2[128]; 
  sprintf(filename2, "psi_6_data/omega%.4f_phi%.1f.dat", omega, rho);
  ...
  for (int index = 0 ; index < 7 ; index++)
  {
    ...
    rdf_count = 0;
    final_psi6 = 0.0;
    for (int count_time = 0; count_time < 499; count_time++){
      if (select_potential==0){sprintf(filename, "position_LJ/N%d_Pe%.1f_omega%.3f_phi%.2f/count_%d.dat", NP, tau_p, omega,rho,count_time);}
      if (select_potential==1){sprintf(filename, "position_WCA/N%d_Pe%.1f_omega%.3f_phi%.2f/count_%d.dat", NP, tau_p, omega,rho,count_time);}
      ...
      //psi_6 /= NP;
      final_psi6 += sqrt(psi_6_real*psi_6_real + psi_6_imag * psi_6_imag)/(double)NP;
  ...
  g_6_Measure(ri,r_host,g_6_host, si, q_host, omega_p_host, omega_t_host, rdf_count, omega, tau_p, rho, select_potential);
  fprintf(data, "%.4f\t%.4f\n", omega, final_psi6/(double)rdf_count);
  }
